...
DependentModel <- function(dataSample, dataSim, mesh, priormaterntype, cov.formula=c("0.5*x + 1.2*y**2"), priormatern, feedbacktype, prior.fixed, prior.hyper.design, prior.mode.location, prior.scale.factor=c(0, 0.1)){
  ...
  Inf.pp.stack <- inla.stack(data = list(y = cbind(NA, y.pp), e = e.pp),
                             A = list(A.pp, 1),
                             effects = list(list(spatial.pp=spde.index$spatial),
                                            list(
                                              Interceptor.pp = rep(1, nv+n),
                                              Cov.pp = c(Cov.pp.mesh, dataSample$cov))
                             ),
                             tag = 'Inference.pp')
  ...
  if(feedbacktype=="moments"){
    ...
    formula <- y ~ -1 + (Interceptor.pp + Cov.pp + f(spatial.pp, copy = 'spatial', fixed = FALSE, hyper = list(beta = gaus.prior))) + (Interceptor + Cov  +  f(spatial, model = spde))
